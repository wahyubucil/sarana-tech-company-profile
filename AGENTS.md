...
_Currently, there is no testing framework initialized in `package.json`._
...
- _(Note: If tests are requested and Vitest is missing, proactively ask the user if you should install it via `bun add -d vitest @testing-library/react jsdom`.)_
...
- **Class Grouping & Sorting:** Tailwind classes are automatically sorted using Prettier (`prettier-plugin-tailwindcss`). Run `bun run format` to sort them.
